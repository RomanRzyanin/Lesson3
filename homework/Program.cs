...
while (true) // true == true
{
    ...
    if (int.TryParse(text, out number)) // == true, строчка состоит из цифр
    {
        ...
        int resEven = CheckNumberEven(number);
        if(resEven % 2 == 0) /*&& sum != 0)*/ //Если прописать, что переменная sum не равна 0, то программа работает, за исключение если введен 0. В программе есть ошибка с инициализацие переменной sum!
        {
            break;
        }
    }
